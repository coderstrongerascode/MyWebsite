...
const flagImage = document.getElementById('pokemon-image');
...
// NEU: Versuchszähler & Highscore
let attemptsLeft = 3;
let highscore = 0;

// NEU: Button für Neues Spiel dynamisch erzeugen
const newGameButton = document.createElement('button');
newGameButton.innerText = 'Neues Spiel';
newGameButton.classList.add('btn', 'hide');
newGameButton.addEventListener('click', startGame);
document.querySelector('.container').appendChild(newGameButton); // Zum Container hinzufügen


// Funktion zum Laden von Daten aus einer JSON-Datei
async function loadQuestions() {
    const response = await fetch('questionpokemon_1_generation.json');
